...
type UserType = 'none' | 'one' | 'multiple';
...
export default async function NewslettersPage({ params }: PageProps) {
  ...
  const hasRight = (newsletter: Newsletter, user: User) => {
    return user.subscriptions.includes(newsletter.subscriptions[0] || '');
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {siteNewsletters.map((newsletter) => (
              <NewsletterCard
                ...
                hasAccess={hasRight(newsletter, userData)}
              />
            ))}
          </div>
        </section>
